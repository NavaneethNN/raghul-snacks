...
export function ProductReviews({ productId }: { productId: number }) {
  ...
  const writeReview = searchParams.get("writeReview") === "1";
  ...
  // Open form and scroll when arriving from the order modal
  useEffect(() => {
    if (!writeReview || !orderId) return;
    setFormOpen(true);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!orderId) { setSubmitError("Order information missing. Please try again from your orders page."); return; }
  ...
  // Nothing to show and no form to open
  if (loading) return null;
  if (reviews.length === 0 && !writeReview) return null;
  ...
  return (
    <section ref={sectionRef} id="reviews" style={{ maxWidth: 820, margin: "48px auto 0", padding: "0 20px 48px", scrollMarginTop: 80 }}>
      ...
      {/* Empty state — only shown when arriving to write a review but no prior reviews exist */}
      {reviews.length === 0 && writeReview && !submitted && (
